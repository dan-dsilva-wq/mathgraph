# =============================================================================
# OVERNIGHT AI EXPERIMENT v4 - Agentic (Self-Improving)
# =============================================================================
# Usage:
#   .\scripts\overnight-v4.ps1                           # Default run
#   .\scripts\overnight-v4.ps1 -MaxLoops 5               # Short test
#   .\scripts\overnight-v4.ps1 -Goal "Make it awesome"   # Goal mode
#   .\scripts\overnight-v4.ps1 -UseVision                # Use vision file
#   .\scripts\overnight-v4.ps1 -DryRun                   # Test without changes
#
...
function Invoke-FixerAI {
    ...
    $fixerPrompt = @"
Fix this specific error in the project. You have full tool access -- read the file, edit it, and save.
...
RULES:
- Read the file, fix the error with Edit, then stop.
- Fix ONLY this error. No refactoring, no improvements.
- Do NOT commit. Just edit the file and leave it for staging.
- If truly impossible, output: BLOCKED: [reason]
"@
    ...
    for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {
        ...
        try {
            $job = Start-Job -ScriptBlock {
                ...
                claude -p --dangerously-skip-permissions --max-turns 10 $p 2>&1
            } -ArgumentList $fixerPrompt, $PROJECT_ROOT
...
"@
    ...
    $fixPrompt = @"
Fix this error in a $techStack project. You have full tool access.
...
RULES:
- Read the file, edit it to fix the error, then stop.
- Fix ONLY this error. No refactoring, no improvements.
- Import paths use @/ alias (e.g. @/lib/supabase/types)
- Do NOT commit. Just edit the file.
    ...
    for ($subLoop = 1; $subLoop -le $MaxSubLoops; $subLoop++) {
        ...
        try {
            $job = Start-Job -ScriptBlock {
                ...
                Set-Location $projRoot
                claude -p --dangerously-skip-permissions --max-turns 10 $p 2>&1
            } -ArgumentList $fixPrompt, $PROJECT_ROOT
...
2. Implement the change (max $maxFiles files)
3. Do NOT commit -- just edit the files and leave them for staging
...
DO THIS:
1. Read the files you want to change
2. Pick ONE meaningful improvement
3. Edit the files to implement it
4. Do NOT commit -- just edit the files and leave them for staging
...
---
*Generated by Overnight AI Experiment v4 - Agentic*
"@
...
function Start-OvernightExperiment {
    ...
    Write-Banner "OVERNIGHT AI EXPERIMENT v4 - Agentic"
    ...
    for ($i = 1; $i -le $MaxLoops; $i++) {
        ...
        for ($retry = 0; $retry -lt $maxRetries; $retry++) {
            try {
                if ($DryRun) {
                    ...
                    # Show spinner while Claude works (v4: agentic mode with tool access)
                    $job = Start-Job -ScriptBlock {
                        param($p, $projRoot)
                        Set-Location $projRoot
                        claude -p --dangerously-skip-permissions --max-turns 25 $p 2>&1
                    } -ArgumentList $prompt, $PROJECT_ROOT
        ...
        if ($null -eq $changedFiles -or $changedFiles.Count -eq 0) {
            if ($claudeFeatureCommits.Count -gt 0) {
                ...
                # Normal no-op -- Claude had tool access but made no changes
                ...
                # Don't count observation-only loops as no-ops
